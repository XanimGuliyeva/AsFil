    ...
    <div class="page-wrapper">
        ...
        <div class="page-breadcrumb">
            <div class="row">
                <div class="col-5 align-self-center">
                    <h4 class="page-title">İstehsalçılar Cədvəli</h4>
                </div>
            </div>
        </div>
        <!-- ============================================================== -->
                                ...
                                <div class="modal-dialog">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form id="main_form">
                                                @csrf
                                                <input type="file" name="sekil" accept="image/*" class="form-control"><br>
                                                <input type="text" name="istehsalci" placeholder="İstehsalçı adı" class="form-control"><br>
                                ...
                                <table class="table">
                                    ...
                                    @foreach($producers as $key=>$producer)
                                            <tr id="{{$producer->id}}">
                                                ...
                                                <td class="text-center">{{$producer->istehsalci}}</td>
                                                <td class="text-center"><input class="producer-img" type="image" src="{{asset('images/'.$producer->sekil.'')}}"></td>
                                                @can('isAdmin')<td class="text-center"><button class="btn btn-info" data-toggle="modal" data-target="#myModal2"><i class="fa fa-pencil"></i></button><br><br><button class="btn btn-danger" href="#myConfirm" data-toggle="modal"><i class="fa fa-trash"></i></button></td>@endcan
                                            </tr>
                                    @endforeach
                                    </tbody>
                                </table>
        ...
        <script>
            ...
            $('.modal-footer').on('click','.btn-dark',function(){
                ...
                var istehsalci = $('input[name=istehsalci]').val().trim();
                $.ajax({
                    type: "POST",
                    url: "add_producer",
                    data: formData,
                    ...
                    success: function (response) {
                        if (response.message == 'success') {
                            ...
                            $('tbody').append('<tr id="' + response.id + '">' +
                                ...
                                '<td class="text-center">' + istehsalci + '</td>\n' +
                                '<td class="text-center"><input class="producer-img" type="image" src="images/' + response.status + '"></td>\n' +
            ...
            $('#myModal2 .modal-footer').on('click','.btn-success',function() {
                ...
                var istehsalci = $('#myModal2 input[name=istehsalci]').val().trim();
                var tr = $('#'+id+'');
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.message == 'success') {
                            toastr.success('İstehsalçı məlumatları yeniləndi');
                            tr.find($('td:eq(0)')).text(istehsalci);
                            if($('#myModal2 input[name=sekil]').get(0).files.length !== 0){
                                tr.find($('.producer-img')).attr('src','images/'+response.file+'');
                            }
                        }
                    },
                    error: function (request, error, response) {
